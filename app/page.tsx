        ...
        {activeTab === 'predict' && (<div>
          ...
          {prediction && teamA && teamB && (<>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 16px', textAlign: 'center' }}><div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', letterSpacing: 1.5, marginBottom: 8 }}>SPREAD</div><div style={{ fontFamily: 'var(--font-display)', fontSize: 38, lineHeight: 1, color: prediction.spread !== 0 ? 'var(--green)' : 'var(--text-primary)' }}>{prediction.spread > 0 ? `${teamAName.split(' ').pop()} -${Math.abs(prediction.spread)}` : prediction.spread < 0 ? `${teamBName.split(' ').pop()} -${Math.abs(prediction.spread)}` : 'PICK'}</div><div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 6 }}>{prediction.spread !== 0 ? `${prediction.spread > 0 ? teamAName : teamBName} favored` : 'Even matchup'}</div></div>
